        ...
        {menuItems.map(({ href, label, ...itemProps }, index) => (
          <li key={href + index} className="flex-auto">
            <NavLink
              {...itemProps}
              href={href}
              className="hover:text-max font-normal hover:-translate-y-px text-foreground-400 [&_span]:hidden w-full justify-center md:[&_span]:block md:[&_span]:left-0 lg:[&_span]:left-2 h-11"
            >
              {label}
            </NavLink>
          </li>
